...
The `block_layouts.csv` / `hall_distances.csv` inputs (and the `layout_<event>.xlsx`
map) are generated by the Python tool in `tools/layout/` — see the README. The Rust
serpentine maths in `src/layout.rs` and the Python expansion in `tools/layout/model.py`
