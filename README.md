...
（距離モデルの調整）、`--time-ms`／`--max-iters`（探索の予算）、`--out-path <json>`（**通過する経路**
をレッグ毎の座標ポリラインとして JSON 出力。地図描画用）。**同じシードでバイト単位まで同一の
...
`--out-path <json>` を渡すと、**通過する経路**をレッグ毎の座標ポリラインとして JSON に書き出します
（地図描画用）。各レッグは `from_space`／`to_space`、`leg_cost`／`cumulative_cost`、`kind`、グローバル
座標 `(x, y)` の `waypoints` を持ちます。`kind` は経路の種類で精度が異なります：

- `intra_island`（同一島）— コストモデルが選んだ通路（交差点）を経由する**正確な**4点ポリライン
  `[a, P1, P2, b]`（机を突き抜けず、最寄りのアイルを回る）。
- `cross_island`／`cross_cluster`（島跨ぎ／クラスタ跨ぎ）— モデルにジオメトリが無い（Manhattan＋
  ペナルティ、またはホール行列の単一値）ため、端点間の**直線近似** `[a, b]`。描画側で破線等に
  描き分けられます。

正確な `intra_island` 経路には島の座標軸ベクトルが必要なため、この機能の追加後は **`gen-layout` を
再実行**して `spaces.json` を作り直してください（軸ベクトルを欠く旧 artifact では島内レッグも直線に
フォールバックします）。
...
`--time-ms`/`--max-iters` (search budget), `--out-path <json>` (write the **walked
route** as per-leg coordinate polylines for drawing on a map). For byte-identical
...
Pass `--out-path <json>` to also write the **walked route** as per-leg coordinate
polylines (for drawing on a map). Each leg carries `from_space`/`to_space`,
`leg_cost`/`cumulative_cost`, a `kind`, and global `(x, y)` `waypoints`. Accuracy
depends on `kind`:

- `intra_island` — an **accurate** 4-point polyline `[a, P1, P2, b]` that rounds the
  exact aisle the cost model picked (you can't cut through the tables).
- `cross_island` / `cross_cluster` — a **straight-segment approximation** `[a, b]`, since
  the model holds no geometry for these legs (Manhattan plus penalties, or a single
  hall-matrix number); a renderer can draw them dashed.

The accurate `intra_island` paths need each island's axis vectors, so after adopting this
feature **re-run `gen-layout`** to rebuild `spaces.json` (legacy artifacts without the
vectors fall back to straight segments for same-island legs too).
